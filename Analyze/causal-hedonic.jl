...
## selection equation
formula_d = DataFrames.Formula(:Y_dummy, :(X + Z))

## with factor:
## formula_d = (Y_dummy ~ (X+F+Z))

mm_d = DataFrames.ModelMatrix( DataFrames.ModelFrame(formula_d, df) );

d_star = mm_d.m*gamma + errs_df[:v];

## potential outcomes
formula_y = DataFrames.Formula(:Y_dummy, :X)
##formula_y = (Y_dummy ~ X)

## with factor:
##formula_y = (Y_dummy ~ X + F)

mm_y = DataFrames.ModelMatrix( DataFrames.ModelFrame(formula_y, df));
...
formula_rhs = ""
for i in x_names
    formula_rhs = join([formula_rhs; string(i)], " + " )
end
formula_rhs = replace(formula_rhs, r"^\s\+\s", "" )

##formula_y = DataFrames.Formula(:Y_obs, parse(formula_rhs) )
formula_y = DataFrames.Formula(:Y_obs, :X)

formula_rhs = join([formula_rhs; z_names], " + " )

##formula_d = DataFrames.Formula(:D_obs, parse(formula_rhs) )
formula_d = DataFrames.Formula(:D_obs, :(X + Z))

ktot = length( beta_true )

## --------------------------------------------------------------------------- #

my, sy = mean(y_obs), std(y_obs)
