...
path.to.data <- "../Data/"
raw.data <- paste0(path.to.data,"Raw/Dataquick/")

# make directory to save data, if necessary
ifelse(!dir.exists(file.path(path.to.data,"Processed")),
       dir.create(file.path(path.to.data,"Processed")), FALSE)
...
path.assr <- paste0(raw.data,"UOI_ASSR_INITIAL.txt")

# "UOI_HIST_INITIAL.txt" - transaction data
path.hist <- paste0(raw.data,"UOI_HIST_INITIAL.txt") 

# "UOI\ Assessor\ Layout.xls" - column names, classes, and widths; I converted xls to csv
path.assr.layout <- paste0(raw.data,"assessor_layout.csv")

# "UOI\ History\ Layout.xls" - column names, classes, and widths; I converted xls to csv
path.hist.layout <- paste0(raw.data,"history_layout.csv")
...
# save
save(assr.laf.fwf.il,file=paste0(path.to.data,"Processed/ILassr.Rda"))
...
# save 
save(hist.laf.fwf.il,file=paste0(path.to.data,"Processed/ILhist.Rda"))
